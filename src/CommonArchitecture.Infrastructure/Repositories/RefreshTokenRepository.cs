...
public class RefreshTokenRepository : IRefreshTokenRepository
{
    ...
    public async Task<bool> DeleteExpiredTokensAsync()
    {
        var expiredTokens = await _context.RefreshTokens
            .Where(rt => rt.ExpiresAt < DateTime.UtcNow || (rt.IsRevoked && rt.ExpiresAt < DateTime.UtcNow.AddDays(-7)))
            .ToListAsync();
    ...
    public async Task<bool> IsTokenReusedAsync(string token, int userId)
    {
        // Check if this token was previously used (stored in PreviousToken field)
        return await _context.RefreshTokens
            .AnyAsync(rt => rt.PreviousToken == token && rt.UserId == userId);
    }
